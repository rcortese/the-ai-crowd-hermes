...
`ops/scripts/hddt-moss.sh` is the fail-closed transaction executor. Production custody is fixed and literal: `the designated private production custody root/{bin,release-source,state}`, with operation, authorization, receipt, lock, retention, and outbox state beneath `state`; shared ancestors are not part of the custody contract. Stack inputs are independently fixed at `the canonical stack root`. `/mnt/user` aliases, symlinks, non-canonical paths, unsafe ownership/modes, domain overlap, and live mount-source overlap are rejected. Only `HDDT_REHEARSAL=1` with an isolated explicit fixture root may select fake paths or fake Docker.
