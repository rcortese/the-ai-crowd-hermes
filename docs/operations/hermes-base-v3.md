...
The source-only lock intentionally leaves `image.final_tag`, `image.final_image_id`, and `receipt` `null`. They are unavailable until a separately authorized host execution writes a receipt.

## Contract and executor

The stdlib contract rejects unrecognized/missing fields, non-exact archive binding, and incomplete receipts. OCI normalization removes **only** `Config.Volumes["/opt/data"]`; all other configuration is copied unchanged. Image-tar normalization applies the equivalent `config.Volumes` deletion and assigns only the supplied final tag.
