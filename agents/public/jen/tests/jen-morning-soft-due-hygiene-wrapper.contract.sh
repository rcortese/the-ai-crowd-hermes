...
if [[ "$*" == *"--dry-run"* ]]; then
  ...
  jq -nc '{contract_version:"jen-morning-due-adjust.v2",status:"ok",mode:"apply",summary:{candidate_count:0,blocked_count:1,write_count:0,skipped_count:0},blocked:[{id:"rec-maint-1",content:"Weekly review",due:{date:"2026-05-28",string:"every week",is_recurring:true},deadline:null,past_due_raw:true,classification:{category:"recurring_maintenance"},signals:["past_due_raw","todoist_recurring_due"]}],audit_log_path:null}'
fi
MOCKWRAP
...
assert_jq "$(cat "$state/apply/morning-recurring-maintenance-reanchor/latest.json")" '.status == "ok" and .boundaries.source_snapshot_reused == true and .summary.write_count == 1 and .candidates[0].id == "rec-maint-1"' 'recurring packet records source snapshot reuse'
