...
case "$*" in
  ...
  "update-due --task-id hard-1 --due 2026-05-29") jq -nc '{status:"ok",command:"update-due",task:{id:"hard-1",due:{date:"2026-05-29"},deadline:{date:"2026-05-30"}},verified:true}' ;;
  "update-due --task-id deadline-open-1 --due 2026-05-29") jq -nc '{status:"ok",command:"update-due",task:{id:"deadline-open-1",due:{date:"2026-05-29"},deadline:{date:"2026-05-30"}},verified:true}' ;;
...
assert_jq "$dry_json" '.summary.total_classified == 7 and .summary.write_count == 0 and .summary.candidate_count == 3 and .summary.blocked_count == 4' 'dry-run summary counts'
...
assert_jq "$dry_json" '.candidates[] | select(.id == "hard-1" and .planned_action == "update-due" and .planned_due == "2026-05-29" and .deadline.date == "2026-05-30" and .classification.category == "hard_deadline")' 'open hard deadline candidate can be surfaced today without changing deadline'
assert_jq "$dry_json" '.candidates[] | select(.id == "deadline-open-1" and .planned_action == "update-due" and .planned_due == "2026-05-29" and .deadline.date == "2026-05-30" and .classification.category == "hard_deadline")' 'Luz-style hard deadline with future deadline can be surfaced today without changing deadline'
assert_jq "$dry_json" '.blocked[] | select(.id == "deadline-passed-1" and .deadline.date == "2026-05-28")' 'hard deadline past its deadline remains blocked/late'
assert_jq "$dry_json" 'all(.blocked[]; .id != "soft-1" and .id != "hard-1" and .id != "deadline-open-1")' 'only soft and still-open deadline-surface candidates are mutable'
...
assert_jq "$apply_json" '.mode == "apply" and .summary.write_count == 3 and ([.writes[].id] | sort == ["deadline-open-1","hard-1","soft-1"]) and all(.writes[]; .operation == "update-due" and .due == "2026-05-29") and .audit_log_path != null' 'enabled apply writes soft candidate and open deadline-surface candidate, with audit path'
[[ "$(grep -c '^update-due --task-id soft-1 --due 2026-05-29$' "$runtime_log")" == "1" ]] || fail "apply must call update-due once for soft candidate"
[[ "$(grep -c '^update-due --task-id hard-1 --due 2026-05-29$' "$runtime_log")" == "1" ]] || fail "apply must call update-due once for open hard deadline candidate"
[[ "$(grep -c '^update-due --task-id deadline-open-1 --due 2026-05-29$' "$runtime_log")" == "1" ]] || fail "apply must call update-due once for Luz-style open deadline candidate"
[[ "$(grep -Ec 'deadline-passed-1|rec-hard-1|rec-maint-1|amb-1' "$runtime_log" || true)" == "0" ]] || fail "apply must not mutate expired-hard/recurring/ambiguous tasks"
...
assert_jq "$repeat_apply_json" '.summary.write_count == 0 and .summary.skipped_count == 3 and .skipped[0].reason == "idempotency_key_already_succeeded"' 'repeat apply is idempotent and performs zero duplicate writes'
[[ "$(grep -c '^update-due --task-id soft-1 --due 2026-05-29$' "$runtime_log")" == "1" ]] || fail "idempotent repeat must not call soft update-due again"
[[ "$(grep -c '^update-due --task-id hard-1 --due 2026-05-29$' "$runtime_log")" == "1" ]] || fail "idempotent repeat must not call open hard deadline update-due again"
[[ "$(grep -c '^update-due --task-id deadline-open-1 --due 2026-05-29$' "$runtime_log")" == "1" ]] || fail "idempotent repeat must not call Luz-style open deadline update-due again"
