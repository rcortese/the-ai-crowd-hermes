...
jq -e '.tasks[] | select(.id == "historical-luz-enel") | select(.final_category == "hard_deadline" and .decision_source == "explicit_deadline" and .writable == true and .skip_reason == null and .evidence.explicit_deadline == true)' <<<"$out" >/dev/null || fail "Luz-Enel hard due explicit deadline and surfaceable before deadline"
