...
assert_jq '.fixtures[] | select(.id == "historical-luz-enel") | select(.content == "Luz - Enel" and .deadline.date == "2026-06-05" and .expected.category == "hard_deadline" and .expected.evidence.explicit_deadline == true and .expected.evidence.decision_source == "explicit_deadline" and .expected.mutation_eligibility.writable_by_morning_hygiene == true and .expected.mutation_eligibility.phase == "deadline_surface" and .expected.mutation_eligibility.skip_reason == null)' 'Luz-Enel remains hard because of explicit deadline, but morning hygiene may surface it up to the deadline'
