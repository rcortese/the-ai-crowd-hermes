...
run_red(){
  ...
  if grep -Eiq 'syntax error|command not found|No such file or directory|unbound variable' "$output"; then
    if [[ $name == render-input-cwd ]]; then
      grep -Fq 'FAKE: render cwd lacks sealed env inputs' "$output" || { show "$output"; fail "$name lacked cwd boundary oracle"; }
    else
...
run_red existing input-drift T51 "$base" hddt 'ASSERT[t51]' '[[ -z ${HDDT_BEFORE_SEAL_HOOK:-} ]]||"$HDDT_BEFORE_SEAL_HOOK"' ': # mutation: omit drift hook'
...
run_red existing staged-source-cleanliness T46 "$base" hddt 'ASSERT[t46]: staged-source-accepted' 'dirty=$("$gb" -c safe.directory="$sr" -C "$sr" status --porcelain)' '"$gb" -c safe.directory="$sr" -C "$sr" diff --quiet||dirty=1' check_source
...
run_red lote-a selector-tag-default T00 "$binding" binding 'binding-tests: RED candidate-selector-accepted' 'grep -qx "MOSS_IMAGE_REF=$expected_image_ref" "$env_file" ||' 'true ||'
...
run_red lote-b recovery-image-only-third T80 "$base" hddt 'ASSERT[t80]: recovery-image-only-third' '.image_id==$live.Image and .container_id==$live.Id and .started_at==$live.State.StartedAt and .restart_count==$live.RestartCount and .expected_running==$live.State.Running' '.image_id==$live.Image and .expected_running==$live.State.Running' candidate_recovery_matches
run_red lote-b cas-ignore-id T55 "$base" hddt 'ASSERT[t55]' 'and .container_id==$live.Id and .started_at==$live.State.StartedAt' 'and .started_at==$live.State.StartedAt' candidate_recovery_matches
run_red lote-b cas-ignore-started-at T56 "$base" hddt 'ASSERT[t56]' 'and .container_id==$live.Id and .started_at==$live.State.StartedAt and .restart_count==$live.RestartCount' 'and .container_id==$live.Id and .restart_count==$live.RestartCount' candidate_recovery_matches
run_red lote-b restart-count-zero T57 "$base" hddt 'ASSERT[t57]' 'elif (.RestartCount//-1)!=0 then "restart_count"' 'elif false then "restart_count"' state_gate
...
# Lote C: full Compose rollback, automatic retention, fleet preservation, and A2A proof.
run_red lote-c split-candidate-rollback-render T84 "$base" hddt 'HDDT: candidate compose snapshot mismatch' 'cp --reflink=never "$release_sr/compose.yaml" "$stage/candidate-inputs/compose.yaml"' 'cp --reflink=never "$stage/rollback-inputs/compose.yaml" "$stage/candidate-inputs/compose.yaml"'
run_red lote-c path-stable-render T84 "$base" hddt 'ASSERT[t84]' 'config --no-path-resolution --format json' 'config --format json'
