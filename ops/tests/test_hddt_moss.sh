...
CAND=sha256:bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb; ROLL=sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa; REV=1111111111111111111111111111111111111111; TREE=2222222222222222222222222222222222222222; REMOTE=ssh://fixture/repo; BASE=fixture/base@sha256:dddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddd; CLOSURE=$(printf 'fixture closure\n'|sha256sum|cut -d' ' -f1); export CAND ROLL BASE
...
case " $* " in *"rev-parse HEAD^{tree}"*) cat $fx/git.tree;; *' rev-parse HEAD'*) cat $fx/git.head;; *'remote get-url origin'*) cat $fx/git.remote;; *' diff --quiet'*) [[ ! -e $fx/git.dirty ]];; *' ls-tree -r --full-tree HEAD '*) printf 'fixture closure\n';; *) exit 91;; esac
EOF
 cat >$BIN/native <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
...
t46(){ touch $FX/git.dirty; rc=0; prepare >/dev/null 2>&1||rc=$?; eq "$rc" 65; zero_effects; has $FX/git.log 'safe.directory='; [[ ! -e $HOME/.gitconfig ]]||lacks $HOME/.gitconfig "$STACK"; }
t47(){
...
if [[ ${1:-} == --case ]]; then [[ $# == 2 && $2 =~ ^T(0[1-9]|[1-7][0-9]|8[01])$ ]]||exit 64; run_case "$2"; exit; fi
suite=${1:-core}; case $suite in core) cases=$(seq -w 1 33);; recovery) cases='34 35 36 37 38 39 40 49';; adapter) cases="47 81";; mutations) cases='41 42 43 44 45 46 48 50 51';; cas) cases=$(seq -w 52 60);; signals) cases=$(seq -w 61 67);; control) cases=$(seq -w 68 73);; oracles) cases=$(seq -w 74 80);; all) cases=$(seq -w 1 81);; *) exit 64;; esac
