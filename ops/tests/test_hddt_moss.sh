...
if [[ ${1:-} == compose ]]; then
 ...
 if [[ " $* " == *' config --no-path-resolution --format json moss '* ]]; then [[ -e $fx/render.fail ]]&&exit 42; grep -q '^extra:' "$fx/stack/compose.yaml" && exit 42; jq -nc --arg image "${MOSS_IMAGE_REF:?}" '{name:"the-ai-crowd",services:{moss:{image:$image,networks:{fixture:null},volumes:[{type:"bind",source:"./runtime/moss-home",target:"/opt/data"},{type:"bind",source:"./agents/public/moss",target:"/agents/moss/public"},{type:"bind",source:"./agents/private/moss",target:"/agents/moss/private"},{type:"bind",source:"./state/shared",target:"/mnt/hermes-shared"},{type:"bind",source:"./agents/private/richmond-archiveops",target:"/archiveops/richmond"},{type:"bind",source:"./state/private/backups",target:"/mnt/user/backups/the-ai-crowd"}]}},networks:{fixture:{external:true}},"x-fixture":{ignored:true}}'; exit; fi
...
case " $* " in *"rev-parse HEAD^{tree}"*) cat $fx/git.tree;; *"rev-parse --verify ${BASE_REV}^{commit}"*) [[ ! -e $fx/git.base.absent ]] || exit 46; printf '%s\n' "$BASE_REV";; *"rev-parse --verify ${REV}^{commit}"*) printf '%s\n' "$REV";; *' rev-parse HEAD'*) cat $fx/git.head;; *"merge-base --is-ancestor $BASE_REV $REV"*) [[ ! -e $fx/git.base.nonancestor ]] || exit 1;; *'remote get-url origin'*) cat $fx/git.remote;; *' diff --quiet'*) [[ ! -e $fx/git.dirty ]];; *' ls-files --error-unmatch -- fixture '*) exit 0;; *' ls-tree -r --full-tree HEAD '*) printf 'fixture closure\n';; *) exit 91;; esac
EOF
 cat >$BIN/native <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
...
t03(){ h1=$(prepare); h2=$(prepare); eq "$h1" "$h2"; no_tmps; jq -e 'keys==["networks","services"] and (.services|keys)==["moss"] and (.networks|keys)==["fixture"] and [.services.moss.volumes[].source]==["./runtime/moss-home","./agents/public/moss","./agents/private/moss","./state/shared","./agents/private/richmond-archiveops","./state/private/backups"] and ([paths(scalars) as $p|getpath($p)|strings]|all(contains("compose-inputs")|not))' "$OP/candidate.rendered.json" >/dev/null||fail normalized-render-contract; has "$FX/argv.log" 'config><--no-path-resolution><--format><json><moss>'; before=$(sha256sum $OP/request.json); rc=0; "$SCRIPT" prepare --operation-id hddt-case-0001 --mode followable --source-revision "$REV" --source-tree "$TREE" --canonical-remote wrong --candidate-image-id "$CAND" --rollback-image-id "$ROLL" --moss-base-image "$BASE" --receipt-sha256 "$RECEIPT_SHA" --approval-context reviewed-fixture >/dev/null 2>&1||rc=$?; eq "$rc" 65; eq "$before" "$(sha256sum $OP/request.json)"; }
...
t10(){ export COMPOSE_FILE=hostile MOSS_IMAGE_REF=hostile; prepare >/dev/null; config_lines=$(grep '<config><--no-path-resolution><--format><json><moss>' $FX/argv.log); [[ $config_lines != *hostile* && $config_lines == *'/staging/hddt-case-0001.'*'/compose-inputs'* ]]||fail inherited-compose-env-or-unsealed-cwd; eq "$(grep -c '^compose.yaml$' $FX/opens.log)" 2; eq "$(grep -c '^\.env$' $FX/opens.log)" 2; eq "$(jq -r .services.moss.image $OP/candidate.rendered.json)" "$CAND"; eq "$(jq -r .services.moss.image $OP/rollback.rendered.json)" "$ROLL"; }
...
t57(){ prepare followable; auth; run_raw >/dev/null 2>&1 & pid=$!; wait_state AWAITING_CONFIRMATION; [[ -f $OP/candidate-identity.json ]]||fail candidate-identity-not-sealed; jq '.RestartCount=1' $LIVE>$LIVE.tmp; mv $LIVE.tmp $LIVE; "$SCRIPT" confirm --operation-id hddt-case-0001; wait $pid||true; state_is RECOVERY_UNRESOLVED; has $OP/journal.log third_state; eq "$(grep -c compose-up $FX/mutations.log)" 1; }
