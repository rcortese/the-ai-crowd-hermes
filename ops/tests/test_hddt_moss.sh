...
fixture(){
 FX=$(mktemp -d "/tmp/hddt-${CASE}.XXXXXX"); STATE=$FX/state; STACK=$FX/stack; BIN=$FX/bin; LIVE=$FX/live.json; CLOCK=$FX/clock; OP=$STATE/operations/hddt-case-0001; mkdir -m700 "$STATE" "$STACK" "$BIN"; mkdir -m700 "$STATE/build-receipts" "$STACK/env"; mkdir -p "$STACK/ops/manifests" "$STACK/ops/scripts"; cp "$ROOT/ops/scripts/build-moss-all-in-one-candidate.sh" "$STACK/ops/scripts/build-moss-all-in-one-candidate.sh"; cp "$ROOT/ops/scripts/hddt-moss-launcher.sh" "$STACK/ops/scripts/hddt-moss-launcher.sh"; chmod 700 "$STACK/ops/scripts"/*.sh; printf '%s\n' fixture >"$STACK/ops/manifests/moss-release-source-closure.paths"; printf '%s\n' fixture >"$STACK/fixture"; printf '100\n'>$CLOCK; printf 'services:\n  moss:\n    image: "${MOSS_IMAGE_REF}"\n'>$STACK/compose.yaml; printf 'FLEET=1\n'>$STACK/.env; printf 'A=1\n'>$STACK/env/fleet.env; printf 'B=1\n'>$STACK/env/moss-webui.env; printf 'C=1\n'>$STACK/env/roy.env; chmod 600 "$STACK/.env" "$STACK/env/"*.env
 ...
 jq -nc '["jen","denholm","roy","richmond","the-elders"] as $names | reduce $names[] as $n ({}; .["the-ai-crowd-"+$n+"-1"]={Id:("peer-"+$n),Image:("sha256:"+("c"*64)),Name:("/the-ai-crowd-"+$n+"-1"),RestartCount:0,State:{Running:true,Status:"running",Health:{Status:"healthy"},StartedAt:("peer-start-"+$n)}})' >$FX/peers.json
 cat >$BIN/docker <<'EOF'
#!/usr/bin/env bash
set -Eeuo pipefail
fx=$(cd "$(dirname "$0")/.."&&pwd); live=$fx/live.json; printf '%s\t' "${HDDT_FAKE_PHASE:-call}" >>$fx/argv.log; printf '<%s>' "$@" >>$fx/argv.log; printf '\tCWD=%s\tENV=' "$PWD" >>$fx/argv.log; env|sort|tr '\n' ',' >>$fx/argv.log; printf '\n' >>$fx/argv.log
...
if [[ ${1:-} == inspect ]]; then
 target=${!#}
 if jq -e --arg target "$target" 'has($target)' "$fx/peers.json" >/dev/null; then
  if [[ -e $fx/after-apply && -e $fx/peer.drift && $target == the-ai-crowd-jen-1 ]]; then jq --arg target "$target" '.[$target].RestartCount=1|.[$target]' "$fx/peers.json"; else jq --arg target "$target" '.[$target]' "$fx/peers.json"; fi
  exit
 fi
...
if [[ ${1:-} == compose ]]; then
 for ((i=1;i<=$#;i++)); do a=${!i}; if [[ $a == -f || $a == --env-file ]]; then j=$((i+1)); opened=${!j}; [[ $opened == /* ]] || opened="$PWD/$opened"; printf '%s\n' "$opened" >>$fx/opens.log; fi; done
 if [[ " $* " == *' config '* && " $* " == *' --format json '* ]]; then
  [[ -f env/fleet.env && -f env/moss-webui.env && -f env/roy.env ]] || { printf '%s\n' 'FAKE: render cwd lacks sealed env inputs' >&2; exit 44; }
  [[ -e $fx/render.fail ]]&&exit 42; grep -q '^extra:' "$fx/stack/compose.yaml" && exit 42
  if [[ -e $fx/render.structural ]]; then
   compose_input=; for ((i=1;i<=$#;i++)); do if [[ ${!i} == -f ]]; then j=$((i+1)); compose_input=${!j}; fi; done
   [[ -n $compose_input && -f $compose_input ]] || exit 43
   read_only=false; grep -q 'read_only: true' "$compose_input" && read_only=true
   jq -nc --arg image "${MOSS_IMAGE_REF:?}" --argjson read_only "$read_only" '{name:"the-ai-crowd",services:{moss:{image:$image,read_only:$read_only,networks:{fleet:null},volumes:[{type:"bind",source:"./runtime/moss-home",target:"/opt/data"},{type:"volume",source:"moss-cache",target:"/cache"}]},jen:{image:"fixture/jen",networks:{fleet:null},volumes:[{type:"volume",source:"jen-cache",target:"/cache"}]}},networks:{fleet:{name:"the-ai-crowd_default"},unused:{name:"unused"}},volumes:{"moss-cache":{name:"the-ai-crowd_moss-cache"},"jen-cache":{name:"the-ai-crowd_jen-cache"}}}'
  else
   jq -nc --arg image "${MOSS_IMAGE_REF:?}" '{services:{moss:{image:$image}}}'
  fi
  exit
 fi
 if [[ " $* " == *' up -d '* ]]; then printf '%s\n' compose-up >>$fx/mutations.log; touch $fx/after-apply; f=; while (($#)); do [[ $1 == -f ]]&&{ f=$2; break; }; shift; done; if [[ $f == "$fx/stack/compose.yaml" ]]; then image=sha256:bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb; else image=$(jq -r .services.moss.image "$f"); fi; [[ -e $fx/apply.fail && $image == sha256:bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb ]]&&exit 43; [[ -e $fx/rollback.fail && $image == sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa ]]&&exit 44; [[ -e $fx/apply.wrong && $image == sha256:bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb ]]&&image=sha256:eeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee; [[ -e $fx/rollback.wrong && $image == sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa ]]&&image=sha256:eeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee; id=$(jq -r .Id "$live"); started=$(jq -r .State.StartedAt "$live"); restarts=0; [[ $image == sha256:bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb && -e $fx/candidate-restarts-one ]]&&restarts=1; [[ $image == sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa && -e $fx/rollback-restarts-one ]]&&restarts=1; jq --arg image "$image" --arg id "$id" --arg started "$started" --argjson restarts "$restarts" '.Image=$image|.Id=$id|.State.Running=true|.State.Status="running"|.State.Health.Status="healthy"|.State.StartedAt=$started|.RestartCount=$restarts' "$live">$live.tmp; mv $live.tmp "$live";
...
case " $* " in *"rev-parse HEAD^{tree}"*) cat $fx/git.tree;; *"rev-parse --verify ${BASE_REV}^{commit}"*) [[ ! -e $fx/git.base.absent ]] || exit 46; printf '%s\n' "$BASE_REV";; *"rev-parse --verify ${REV}^{commit}"*) printf '%s\n' "$REV";; *' rev-parse HEAD'*) cat $fx/git.head;; *"merge-base --is-ancestor $BASE_REV $REV"*) [[ ! -e $fx/git.base.nonancestor ]] || exit 1;; *'remote get-url origin'*) cat $fx/git.remote;; *' status --porcelain'*) [[ ! -e $fx/git.staged ]] || printf 'M  compose.yaml\n'; [[ ! -e $fx/git.dirty ]] || printf ' M compose.yaml\n';; *' diff --quiet'*) [[ ! -e $fx/git.dirty ]];; *' ls-files --error-unmatch -- fixture '*) exit 0;; *' ls-tree -r --full-tree HEAD '*) printf 'fixture closure\n';; *) exit 91;; esac
EOF
 cat >$BIN/native <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
printf '<%s>' "$@" >>"$FAKE_ROOT/native.log"; printf '\n' >>"$FAKE_ROOT/native.log"
[[ ! -e $FAKE_ROOT/native.fail && ! -e $FAKE_ROOT/native.cleanup-fail ]] || exit 1
if [[ -e $FAKE_ROOT/native.candidate-fail && $(jq -r .Image "$FAKE_ROOT/live.json") == "$CAND" ]]; then exit 1; fi
EOF
 cat >$BIN/sleep <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
...
EOF
 ...
 export HDDT_REHEARSAL=1 HDDT_STATE_ROOT=$STATE HDDT_STACK_ROOT=$STACK HDDT_DOCKER_BIN=$BIN/docker HDDT_CURL_BIN=$BIN/curl HDDT_GIT_BIN=$BIN/git HDDT_NATIVE_ADAPTER=$BIN/native HDDT_A2A_BIN=$BIN/native HDDT_CUSTODY_UID=$(id -u) HDDT_CLOCK_FILE=$CLOCK HDDT_SLEEP_BIN=$BIN/sleep HDDT_SLEEP_SECONDS=0 HDDT_PROBE_SECONDS=4 HDDT_CONFIRMATION_SECONDS=20 FAKE_ROOT=$FX HDDT_CANDIDATE_IMAGE_ID=$CAND HDDT_EXPECTED_NETWORK=fixture
 chmod 700 "$STATE" "$STACK"; receipt
}
receipt(){ local f=$STATE/build-receipts/sha256-${CAND#sha256:}.json exec; exec=$(sha256sum "$SCRIPT"|cut -d' ' -f1); builder=$(sha256sum "$STACK/ops/scripts/build-moss-all-in-one-candidate.sh"|cut -d' ' -f1); launcher=$(sha256sum "$STACK/ops/scripts/hddt-moss-launcher.sh"|cut -d' ' -f1); jq -ncS --arg rev "$REV" --arg source_base "$BASE_REV" --arg tree "$TREE" --arg remote "$REMOTE" --arg image "$CAND" --arg base "$BASE" --arg closure "$CLOSURE" --arg exec "$exec" --arg builder "$builder" --arg launcher "$launcher" '{source_revision:$rev,source_base_revision:$source_base,source_tree:$tree,source_remote:$remote,source_closure_sha256:$closure,candidate_image_id:$image,base_image:$base,context_sha256:"dddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddd",agent_source_revision:$rev,agent_source_tree:$tree,webui_source_revision:$rev,webui_source_tree:$tree,builder_sha256:$builder,executor_sha256:$exec,launcher_sha256:$launcher,toolchain_sha256:"eeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee",created_epoch:1}' >$f; chmod 600 $f; RECEIPT_SHA=$(sha256sum $f|cut -d' ' -f1); }
prepare(){ "$SCRIPT" prepare --operation-id hddt-case-0001 --mode "${1:-followable}" --source-revision "$REV" --source-tree "$TREE" --canonical-remote "$REMOTE" --candidate-image-id "$CAND" --rollback-image-id "$ROLL" --moss-base-image "$BASE" --receipt-sha256 "$RECEIPT_SHA" --approval-context reviewed-fixture; }
auth(){ load_req; jq -ncS --arg op hddt-case-0001 --arg req "$request_sha256" --arg candidate "$CAND" --arg rollback "$ROLL" --arg base "$BASE" --arg rev "$REV" --arg tree "$TREE" --arg remote "$REMOTE" --arg cand "$candidate_render_sha256" --arg roll "$rollback_render_sha256" --arg builder "$(jq -r .builder_sha256 "$OP/candidate-provenance.json")" --arg exec "$executor_sha256" --arg launcher "$(jq -r .launcher_sha256 "$OP/candidate-provenance.json")" '{operation_id:$op,request_sha256:$req,candidate_image_id:$candidate,rollback_image_id:$rollback,moss_base_image:$base,source_revision:$rev,source_tree:$tree,source_remote:$remote,candidate_render_sha256:$cand,rollback_render_sha256:$roll,builder_sha256:$builder,executor_sha256:$exec,launcher_sha256:$launcher,operations:["run"],consumed:false,approval_id:"approval-0001",approval_channel:"fixture",approved_epoch:1,expires_epoch:999999}' >$STATE/authorizations/hddt-case-0001.ready; chmod 600 $STATE/authorizations/hddt-case-0001.ready; }
...
run_raw(){ "$SCRIPT" run --operation-id hddt-case-0001; }
run(){
  "$SCRIPT" run --operation-id hddt-case-0001 >/dev/null 2>&1 & local pid=$! state
  for _ in {1..500}; do
    state=$(awk -F '\t' 'END{print $2}' "$OP/journal.log" 2>/dev/null || true)
    if [[ $state == AWAITING_CONFIRMATION ]]; then "$SCRIPT" confirm --operation-id hddt-case-0001 --reason fixture-auto-confirm >/dev/null 2>&1 || true; break; fi
    [[ -f $OP/terminal.json ]] && break
    /usr/bin/sleep .01
  done
  wait "$pid"
}
...
preapply_snapshot(){
  local request; request=$(jq -r .request_sha256 "$OP/request.json")
  jq --arg request "$request" '{request_sha256:$request,container_id:.Id,image_id:.Image,running:.State.Running,status:.State.Status,health:(.State.Health.Status//"unavailable"),started_at:.State.StartedAt,restart_count:.RestartCount}' "$LIVE" >"$OP/snapshot.json"
  jq -cS --arg request "$request" '{request_sha256:$request,peers:(to_entries|map({key:.key,value:{container_id:.value.Id,image_id:.value.Image,started_at:.value.State.StartedAt,restart_count:.value.RestartCount,running:.value.State.Running,status:.value.State.Status,health:(.value.State.Health.Status//"unavailable")}})|from_entries)}' "$FX/peers.json" >"$OP/fleet-snapshot.json"
  chmod 600 "$OP/snapshot.json" "$OP/fleet-snapshot.json"
}
...
t03(){ h1=$(prepare); h2=$(prepare); eq "$h1" "$h2"; no_tmps; before=$(sha256sum $OP/request.json); rc=0; "$SCRIPT" prepare --operation-id hddt-case-0001 --mode followable --source-revision "$REV" --source-tree "$TREE" --canonical-remote wrong --candidate-image-id "$CAND" --rollback-image-id "$ROLL" --moss-base-image "$BASE" --receipt-sha256 "$RECEIPT_SHA" --approval-context reviewed-fixture >/dev/null 2>&1||rc=$?; eq "$rc" 65; eq "$before" "$(sha256sum $OP/request.json)"; }
...
t10(){ export COMPOSE_FILE=hostile MOSS_IMAGE_REF=hostile; prepare >/dev/null; config_lines=$(grep '<config><--no-path-resolution><--format><json>' $FX/argv.log); eq "$(printf '%s\n' "$config_lines"|grep -c '^call')" 2; [[ $config_lines != *hostile* ]]||fail inherited-compose-env; grep -Eq '/staging/hddt-case-0001\.[^/]+/candidate-inputs/compose.yaml$' $FX/opens.log||fail sealed-candidate-compose-open; grep -Eq '/staging/hddt-case-0001\.[^/]+/rollback-inputs/compose.yaml$' $FX/opens.log||fail sealed-rollback-compose-open; eq "$(jq -r .services.moss.image $OP/candidate.rendered.json)" "$CAND"; eq "$(jq -r .services.moss.image $OP/rollback.rendered.json)" "$ROLL"; }
...
t21(){ rc=0; prepare automatic >/dev/null 2>&1||rc=$?; [[ $rc == 65 || $rc == 64 ]]||fail automatic-accepted; zero_effects; }
t22(){ rc=0; prepare automatic >/dev/null 2>&1||rc=$?; [[ $rc == 65 || $rc == 64 ]]||fail automatic-accepted; zero_effects; }
t23(){ rc=0; prepare automatic >/dev/null 2>&1||rc=$?; [[ $rc == 65 || $rc == 64 ]]||fail automatic-accepted; zero_effects; }
t24(){ prepare followable; auth; run_raw >/dev/null 2>&1 & pid=$!; wait_state AWAITING_CONFIRMATION; "$SCRIPT" confirm --operation-id hddt-case-0001 --reason reviewed; wait $pid; state_is SUCCEEDED; has $OP/journal.log CONFIRMING; [[ -f $OP/control/decision.consumed ]]; }
t25(){ prepare followable; auth; run_raw >/dev/null 2>&1 & pid=$!; wait_state AWAITING_CONFIRMATION; "$SCRIPT" rollback --operation-id hddt-case-0001 --reason operator; wait $pid||true; state_is ROLLED_BACK; has $OP/journal.log requested; }
t26(){ prepare followable; auth; rc=0; run_raw >/dev/null 2>&1||rc=$?; [[ $rc != 0 ]]; state_is ROLLED_BACK; has $OP/journal.log confirmation_timeout; eq "$(grep -c $'ROLLING_BACK\tconfirmation_timeout' $OP/journal.log)" 1; }
t27(){ prepare followable; rc=0; "$SCRIPT" confirm --operation-id hddt-case-0001 >/dev/null 2>&1||rc=$?; eq "$rc" 65; zero_effects; auth; run_raw >/dev/null 2>&1 & pid=$!; wait_state AWAITING_CONFIRMATION; "$SCRIPT" confirm --operation-id hddt-case-0001; wait $pid; rc=0; "$SCRIPT" confirm --operation-id hddt-case-0001 >/dev/null 2>&1||rc=$?; eq "$rc" 65; state_is SUCCEEDED; }
t28(){ prepare followable; auth; run_raw >/dev/null 2>&1 & pid=$!; wait_state AWAITING_CONFIRMATION; touch $FX/probe.health-8787.fail; "$SCRIPT" confirm --operation-id hddt-case-0001; wait $pid||true; state_is ROLLED_BACK; has $OP/journal.log confirm_readback_failed; }
t29(){ prepare; auth; touch $FX/probe.host-health-8644.fail; run_raw >/dev/null 2>&1||true; state_is ROLLED_BACK; has $OP/journal.log probe_host-health-8644; has $OP/journal.log ROLLED_BACK; has $FX/probes.log $'host\t-\thttp://127.0.0.1:8644/health\thost'; }
t30(){ prepare; auth; touch $FX/probe.health-8644.fail $FX/rollback.fail; run_raw >/dev/null 2>&1||true; state_is ROLLBACK_FAILED; eq "$(grep -c compose-up $FX/mutations.log)" 2; lacks $FX/argv.log '<tag>'; }
t31(){ prepare; auth; touch $FX/probe.health-8644.fail $FX/rollback.wrong; run_raw >/dev/null 2>&1||true; state_is ROLLBACK_FAILED; eq "$(jq -r .Image $LIVE)" sha256:eeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee; }
...
t34(){ prepare; auth; hook=$BIN/kill-after-intent; printf '#!/usr/bin/env bash\nkill -KILL "$PPID"\n' >$hook; chmod +x $hook; export HDDT_AFTER_APPLY_INTENT_HOOK=$hook; rc=0; run_raw >/dev/null 2>&1||rc=$?; unset HDDT_AFTER_APPLY_INTENT_HOOK; eq "$rc" 137; "$SCRIPT" recover --operation-id hddt-case-0001; state_is REJECTED_PRE_APPLY; eq "$(grep -c compose-up $FX/mutations.log)" 0; }
...
t43(){ prepare followable; auth; run_raw >/dev/null 2>&1 & pid=$!; wait_state AWAITING_CONFIRMATION; ( "$SCRIPT" confirm --operation-id hddt-case-0001 --reason first >/dev/null 2>&1 ) & a=$!; ( "$SCRIPT" rollback --operation-id hddt-case-0001 --reason second >/dev/null 2>&1 ) & b=$!; wait $a||true; wait $b||true; wait $pid||true; [[ -f $OP/terminal.json ]]; eq "$(find $OP/control -name 'decision.*'|wc -l)" 1; eq "$(grep -Ec $'\t(SUCCEEDED|ROLLED_BACK)\t' $OP/journal.log)" 1; }
t44(){ prepare; auth; jq '.operations=["run","any"]' $STATE/authorizations/hddt-case-0001.ready>$FX/a; mv $FX/a $STATE/authorizations/hddt-case-0001.ready; chmod 600 $STATE/authorizations/hddt-case-0001.ready; run_raw >/dev/null 2>&1||true; state_is REJECTED_PRE_APPLY; zero_effects; [[ -f $STATE/authorizations/hddt-case-0001.ready ]]; }
...
t46(){ touch $FX/git.dirty; rc=0; prepare >/dev/null 2>&1||rc=$?; eq "$rc" 65; zero_effects; rm -f $FX/git.dirty; touch $FX/git.staged; rc=0; prepare >/dev/null 2>&1||rc=$?; [[ $rc == 65 ]]||fail staged-source-accepted; zero_effects; has $FX/git.log 'safe.directory='; home=${HOME:-}; [[ -z $home || ! -e $home/.gitconfig ]]||lacks "$home/.gitconfig" "$STACK"; }
t47(){
...
t55(){ prepare followable; auth; run_raw >/dev/null 2>&1 & pid=$!; wait_state AWAITING_CONFIRMATION; [[ -f $OP/candidate-identity.json ]]||fail candidate-identity-not-sealed; jq '.Id="confirm-id-drift"' $LIVE>$LIVE.tmp; mv $LIVE.tmp $LIVE; "$SCRIPT" confirm --operation-id hddt-case-0001; wait $pid||true; state_is RECOVERY_UNRESOLVED; has $OP/journal.log third_state; eq "$(grep -c compose-up $FX/mutations.log)" 1; }
t56(){ prepare followable; auth; run_raw >/dev/null 2>&1 & pid=$!; wait_state AWAITING_CONFIRMATION; [[ -f $OP/candidate-identity.json ]]||fail candidate-identity-not-sealed; jq '.State.StartedAt="confirm-start-drift"' $LIVE>$LIVE.tmp; mv $LIVE.tmp $LIVE; "$SCRIPT" confirm --operation-id hddt-case-0001; wait $pid||true; state_is RECOVERY_UNRESOLVED; has $OP/journal.log third_state; eq "$(grep -c compose-up $FX/mutations.log)" 1; }
t57(){ prepare; auth; touch "$FX/candidate-restarts-one" "$FX/rollback-restarts-one"; rc=0; run_raw >/dev/null 2>&1||rc=$?; [[ $rc != 0 ]]||fail stable-nonzero-restart-admitted; state_is ROLLBACK_FAILED; has "$OP/journal.log" probe_restart_count; eq "$(grep -c compose-up "$FX/mutations.log")" 2; eq "$(jq -r .RestartCount "$LIVE")" 1; }
...
t61(){ prepare; auth; touch "$FX/live.block"; rc=0; run_raw >"$FX/run.out" 2>&1||rc=$?; eq "$rc" 143; assert_signal_terminal REJECTED_PRE_APPLY; zero_effects; [[ -f $STATE/authorizations/hddt-case-0001.ready && ! -e $STATE/authorizations/hddt-case-0001.consumed ]]||fail preapply-authorization-consumed; }
t62(){ prepare; auth; hook=$BIN/signal-after-intent; printf '#!/usr/bin/env bash\nkill -TERM "$PPID"\n' >"$hook"; chmod +x "$hook"; export HDDT_AFTER_APPLY_INTENT_HOOK=$hook; rc=0; run_raw >"$FX/run.out" 2>&1||rc=$?; unset HDDT_AFTER_APPLY_INTENT_HOOK; eq "$rc" 143; assert_signal_terminal ROLLED_BACK; eq "$(grep -c compose-up "$FX/mutations.log")" 0; [[ -f $STATE/authorizations/hddt-case-0001.consumed && ! -e $STATE/authorizations/hddt-case-0001.ready ]]||fail postapply-authorization-not-consumed; }
t63(){ prepare; auth; touch "$FX/apply.block"; rc=0; run_raw >"$FX/run.out" 2>&1||rc=$?; eq "$rc" 130; assert_signal_terminal ROLLED_BACK; eq "$(grep -c compose-up "$FX/mutations.log")" 2; }
t64(){ prepare; auth; touch "$FX/probe.health-8787.fail"; rc=0; HDDT_REAL_SIGNAL_SLEEP=1 run_raw >"$FX/run.out" 2>&1||rc=$?; eq "$rc" 143; assert_signal_terminal ROLLED_BACK; eq "$(grep -c compose-up "$FX/mutations.log")" 2; }
t65(){ prepare followable; auth; touch "$FX/signal.await"; rc=0; run_raw >"$FX/run.out" 2>&1||rc=$?; eq "$rc" 130; assert_signal_terminal ROLLED_BACK; eq "$(grep -c compose-up "$FX/mutations.log")" 2; }
t66(){ prepare; auth; hook=$BIN/signal-third; printf '#!/usr/bin/env bash\njq '\'' .Image="sha256:eeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee" '\'' "$HDDT_TEST_LIVE" >"$HDDT_TEST_LIVE.tmp"; mv "$HDDT_TEST_LIVE.tmp" "$HDDT_TEST_LIVE"\nkill -TERM "$PPID"\n' >"$hook"; chmod +x "$hook"; export HDDT_TEST_LIVE=$LIVE HDDT_AFTER_APPLY_INTENT_HOOK=$hook; rc=0; run_raw >"$FX/run.out" 2>&1||rc=$?; unset HDDT_TEST_LIVE HDDT_AFTER_APPLY_INTENT_HOOK; eq "$rc" 143; assert_signal_terminal RECOVERY_UNRESOLVED; zero_effects; has "$OP/journal.log" third_state; }
t67(){ prepare; auth; hook=$BIN/error-after-intent; printf '#!/usr/bin/env bash\nexit 73\n' >"$hook"; chmod +x "$hook"; export HDDT_AFTER_APPLY_INTENT_HOOK=$hook; rc=0; run_raw >"$FX/run.out" 2>&1||rc=$?; unset HDDT_AFTER_APPLY_INTENT_HOOK; [[ $rc != 0 ]]||fail err-hook-succeeded; assert_signal_terminal ROLLED_BACK; zero_effects; }
...
t84(){
  local release=$FX/release
  cp -a "$STACK" "$release"
  printf 'services:\n  moss:\n    image: "${MOSS_IMAGE_REF}"\n    read_only: false\n' >"$STACK/compose.yaml"
  printf 'services:\n  moss:\n    image: "${MOSS_IMAGE_REF}"\n    read_only: true\n' >"$release/compose.yaml"
  export HDDT_RELEASE_SOURCE_ROOT=$release
  touch "$FX/render.structural"
  prepare >/dev/null
  for rendered in "$OP/candidate.rendered.json" "$OP/rollback.rendered.json"; do
    eq "$(jq -r '.services|keys|join(",")' "$rendered")" moss
    eq "$(jq -r '.networks|keys|join(",")' "$rendered")" fleet
    eq "$(jq -r '.volumes|keys|join(",")' "$rendered")" moss-cache
    eq "$(jq -r '.services.moss.volumes[]|select(.type=="bind")|.source' "$rendered")" ./runtime/moss-home
    jq -e 'has("name")|not' "$rendered" >/dev/null || fail project-name-leaked
    if jq -e '..|strings|select(contains("compose-inputs") or contains("candidate-inputs") or contains("rollback-inputs"))' "$rendered" >/dev/null; then fail sealed-stage-path-leaked; fi
  done
  eq "$(jq -r .services.moss.read_only "$OP/candidate.rendered.json")" true
  eq "$(jq -r .services.moss.read_only "$OP/rollback.rendered.json")" false
  has "$FX/argv.log" '<--no-path-resolution>'
  auth
  run
  state_is SUCCEEDED
  has "$FX/opens.log" "$OP/candidate.rendered.json"
  lacks "$FX/opens.log" "$STACK/compose.yaml"
  has "$FX/argv.log" "<--project-directory><$STACK>"
}
t85(){
  local expired=$STATE/operations/hddt-expired-0001 active=$STATE/operations/hddt-active-0001
  mkdir -p "$STATE/operations" "$STATE/authorizations" "$STATE/outbox"; chmod 700 "$STATE/operations" "$STATE/authorizations" "$STATE/outbox"
  mkdir -m 700 -p "$expired" "$active"
  jq -nc '{request_sha256:"eeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee",delete_after_epoch:99}' >"$expired/retention.json"
  jq -nc '{request_sha256:"eeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee"}' >"$expired/request.json"
  jq -nc '{state:"ROLLED_BACK",reason:"fixture",created_epoch:1}' >"$expired/terminal.json"
  jq -nc '{request_sha256:"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",delete_after_epoch:99}' >"$active/retention.json"
  chmod 600 "$expired/"*.json "$active/retention.json"
  printf fixture >"$STATE/authorizations/hddt-expired-0001.consumed"; chmod 600 "$STATE/authorizations/hddt-expired-0001.consumed"
  printf fixture >"$STATE/outbox/hddt-expired-0001.ready"; chmod 600 "$STATE/outbox/hddt-expired-0001.ready"
  HDDT_REHEARSAL=1 HDDT_ROOT="$STATE" HDDT_STACK_ROOT="$STACK" HDDT_RELEASE_SOURCE_ROOT="$STACK" HDDT_DOCKER_BIN="$BIN/docker" HDDT_GIT_BIN="$BIN/git" HDDT_CURL_BIN="$BIN/curl" HDDT_A2A_BIN="$BIN/a2a" HDDT_CLOCK_FILE="$CLOCK" "$SCRIPT" prune >/dev/null
  [[ ! -e $expired && ! -e $STATE/authorizations/hddt-expired-0001.consumed && ! -e $STATE/outbox/hddt-expired-0001.ready ]] || fail expired-terminal-retained
  [[ -d $active && -f $active/retention.json ]] || fail active-operation-pruned
  zero_effects
}
t86(){
  prepare >/dev/null; auth; touch "$FX/peer.drift"
  rc=0; run_raw >/dev/null 2>&1 || rc=$?
  [[ $rc != 0 ]] || fail peer-drift-accepted
  state_is ROLLBACK_FAILED
  has "$OP/journal.log" peer_drift
  eq "$(jq '.peers|length' "$OP/fleet-snapshot.json")" 5
  eq "$(grep -c compose-up "$FX/mutations.log")" 2
}
t87(){
  prepare >/dev/null; auth; touch "$FX/native.candidate-fail"
  rc=0; run_raw >/dev/null 2>&1 || rc=$?
  [[ $rc != 0 ]] || fail a2a-failure-accepted
  state_is ROLLED_BACK
  has "$OP/journal.log" probe_a2a
  has "$FX/native.log" '<--from><moss><--to><jen>'
  has "$FX/native.log" '<--from><jen><--to><moss>'
}
run_case(){
...
if [[ ${1:-} == --case ]]; then [[ $# == 2 && $2 =~ ^T(0[1-9]|[1-7][0-9]|8[0-7])$ ]]||exit 64; run_case "$2"; exit; fi
suite=${1:-core}; case $suite in core) cases=$(seq -w 1 33);; recovery) cases='34 35 36 37 38 39 40 49';; adapter) cases="47 81";; mutations) cases='41 42 43 44 45 46 48 50 51';; cas) cases=$(seq -w 52 60);; signals) cases=$(seq -w 61 67);; control) cases=$(seq -w 68 73);; oracles) cases='74 75 76 77 78 79 80 82 83';; compose-rollback) cases='84 85 86 87';; all) cases=$(seq -w 1 87);; *) exit 64;; esac
