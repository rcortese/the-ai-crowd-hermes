...
private_input="$fx/private-node-input"
...
grep -Fq "<--build-context><clash_royale_build_input=$private_input>" "$docker_log" || fail 'hash-verified private build input was not bound'
...
printf '%s\n' 'moss-candidate-build-contract: PASS real-git=true docker=fake source-base=true immutable-base=true private-node-input-hash=true executor-binding=true executor-mutant=RED idempotent=true negatives=7'
