...
launcher_err=$(mktemp /tmp/hddt-launcher-init.err.XXXXXX); launcher_out=$(mktemp /tmp/hddt-launcher-init.out.XXXXXX); trap 'rm -f -- "$launcher_err" "$launcher_out"' EXIT
set +e
"$launcher" >"$launcher_out" 2>"$launcher_err"
launcher_rc=$?
set -e
[[ $launcher_rc == 64 ]] || fail "launcher initialization rc=$launcher_rc: $(<"$launcher_err")"
[[ ! -s $launcher_out ]] || fail launcher-initialization-stdout
[[ $(<"$launcher_err") == 'HDDT launcher: usage: --operation-id ID' && $(wc -l <"$launcher_err") == 1 ]] || fail launcher-initialization-diagnostic
