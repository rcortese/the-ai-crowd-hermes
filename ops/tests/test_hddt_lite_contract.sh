...
grep -Fq '(umask 077; exec env -i HOME=/root PATH=/usr/bin:/bin nohup setsid' "$launcher" || fail launcher-umask-process
if grep -Fq 'PATH=/usr/bin:/bin umask' "$launcher"; then fail launcher-umask-executable; fi
