...
usage: build-moss-integrated-release.sh \
  ...
  --builder-repo GIT_DIR --builder-rev COMMIT \
  --moss-rev COMMIT --receipt FILE
USAGE
  exit 64
}

TAG= BASE_IMAGE= AGENT_REPO= AGENT_REV= WEBUI_REPO= WEBUI_REV=
BUILDER_REPO= BUILDER_REV= MOSS_REV= RECEIPT=
while (($#)); do
  case "$1" in
    --tag) TAG=${2:-}; shift 2 ;;
...
[[ -n $TAG && -n $BASE_IMAGE && -n $AGENT_REPO && -n $AGENT_REV && \
   -n $WEBUI_REPO && -n $WEBUI_REV && -n $BUILDER_REPO && \
   -n $BUILDER_REV && -n $MOSS_REV && -n $RECEIPT ]] || usage
[[ $BASE_IMAGE =~ ^sha256:[0-9a-f]{64}$ ]] || { echo 'base image must be an immutable local image ID' >&2; exit 65; }
for rev in "$AGENT_REV" "$WEBUI_REV" "$BUILDER_REV" "$MOSS_REV"; do
...
cleanup() {
  if [[ $base_alias_created == true ]]; then "$DOCKER_BIN" image rm "$BASE_ALIAS" >/dev/null 2>&1 || true; fi
  rm -rf -- "$tmp"
}
trap cleanup EXIT
...
"$PYTHON_BIN" - "$RECEIPT" "$IMAGE_ID" "$BASE_IMAGE" \
  "$AGENT_REV" "$AGENT_TREE" "$AGENT_ARCHIVE_SHA" "$AGENT_MARKER_SHA" \
  "$WEBUI_REV" "$WEBUI_TREE" "$WEBUI_ARCHIVE_SHA" "$WEBUI_MARKER_SHA" \
  "$BUILDER_REV" "$MOSS_REV" "$DOCKERFILE_SHA" "$running_builder_sha" <<'PY'
import json, os, pathlib, sys, tempfile
(p,image,base,agent_rev,agent_tree,agent_archive,agent_marker,
 webui_rev,webui_tree,webui_archive,webui_marker,builder_rev,moss_rev,
