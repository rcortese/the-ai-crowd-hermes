#!/usr/bin/env bash
set -Eeuo pipefail
usage() { echo "usage: $0 --compose-root DIR --env-file FILE --expected-image-ref sha256:... --expected-rollback-image-ref sha256:..." >&2; exit 64; }
compose_root= env_file= expected_image_ref= expected_rollback_image_ref=
while (($#)); do case "$1" in
 --compose-root|--env-file|--expected-image-ref|--expected-rollback-image-ref) [[ $# -gt 1 ]] || usage; case "$1" in --compose-root) compose_root=$2;; --env-file) env_file=$2;; --expected-image-ref) expected_image_ref=$2;; --expected-rollback-image-ref) expected_rollback_image_ref=$2;; esac; shift 2;; *) usage;; esac; done
