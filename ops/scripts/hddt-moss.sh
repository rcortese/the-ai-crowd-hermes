...
copy_inputs(){ local sr=$1 dst=$2 f; mkdir -m 700 -p "$dst/env"; for f in compose.yaml .env env/fleet.env env/moss-webui.env; do [[ -f $sr/$f && ! -L $sr/$f ]]||die "unsafe input $f" 65; cp --reflink=never "$sr/$f" "$dst/$f"; chmod 600 "$dst/$f"; done; # Compose validates every declared env_file before service selection. Supply a private empty parser placeholder; never copy the remote persona's secrets.
 : >"$dst/env/roy.env"; chmod 600 "$dst/env/roy.env"; }
render(){ local op=$1 kind=$2 image=$3 db out raw; db=$(docker_bin); out="$op/$kind.rendered.json"; raw="$out.raw.tmp"; (cd "$op/compose-inputs"; env -i HOME=/root PATH="$PATH" MOSS_IMAGE_REF="$image" "$db" compose --env-file .env --project-directory . --project-name "$PROJECT" -f compose.yaml config --no-path-resolution --format json "$SERVICE") >"$raw"; jq -cS '{services:{moss:.services.moss},networks:(.networks//{})}' "$raw" >"$out.tmp"; rm -f "$raw"; jq -e --arg image "$image" 'keys==["networks","services"] and (.services|keys)==["moss"] and .services.moss.image==$image and (.services.moss|has("build")|not) and ((.services.moss.networks|keys)-(.networks|keys)|length)==0 and ([paths(scalars) as $p|getpath($p)|strings]|all((contains("compose-inputs") or contains("${") or contains("env_file"))|not))' "$out.tmp" >/dev/null||die "invalid $kind render" 65; chmod 600 "$out.tmp"; sync "$out.tmp"; mv "$out.tmp" "$out"; sync "$op"; sha "$out"; }
prepare(){
 ...
 local r release_sr stack_sr op stage closure receipt before after cand_hash roll_hash input_hash exec_hash builder_hash launcher_hash body req_hash; r=$(root); release_sr=$(release_source); stack_sr=$(stack_inputs); init_root "$r"; op="$r/operations/$operation_id"
 ...
 before=$(input_manifest "$stack_sr"); copy_inputs "$stack_sr" "$stage/compose-inputs"; [[ -z ${HDDT_BEFORE_SEAL_HOOK:-} ]]||"$HDDT_BEFORE_SEAL_HOOK"; after=$(input_manifest "$stack_sr"); [[ $before == "$after" ]]||die 'live inputs drifted before seal' 65
...
apply_render(){ local op=$1 kind=$2 expected hashvar db; expected=$([[ $kind == candidate ]]&&printf %s "$candidate_image_id"||printf %s "$rollback_image_id"); hashvar=$([[ $kind == candidate ]]&&printf %s "$candidate_render_sha256"||printf %s "$rollback_render_sha256"); [[ $(sha "$op/$kind.rendered.json") == "$hashvar" ]]||die "$kind render hash drift" 65; jq -e --arg image "$expected" '.services.moss.image==$image' "$op/$kind.rendered.json" >/dev/null||die "$kind render binding drift" 65; db=$(docker_bin); env -i HOME=/root PATH="$PATH" "$db" compose --project-directory "$DEFAULT_STACK_INPUTS" --project-name "$PROJECT" -f "$op/$kind.rendered.json" up -d --no-build --no-deps --force-recreate "$SERVICE"; }
...
recover(){
 ...
 APPLYING|VERIFYING_BASE|CONFIRMING) if [[ $decision == rollback ]]; then rollback_live "$op" recovery_requested; elif [[ $decision == confirm && $class == candidate ]]&&probe_once "$op" candidate >/dev/null; then journal "$op" SUCCEEDED recovered_confirm; terminal "$op" SUCCEEDED recovered_confirm; elif [[ $class == candidate ]]&&probe_once "$op" candidate >/dev/null; then journal "$op" SUCCEEDED recovered_candidate; terminal "$op" SUCCEEDED recovered_candidate; elif [[ $(rollback_relation "$op" "$j") == rollback ]]; then journal "$op" ROLLED_BACK recovered_rollback; terminal "$op" ROLLED_BACK recovered_rollback; else journal "$op" RECOVERY_UNRESOLVED third_state; terminal "$op" RECOVERY_UNRESOLVED third_state; fi;;
