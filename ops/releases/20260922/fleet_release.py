...
STATE=STACK/'state/private/backups/fleet-release-20260922'
...
def check(m,idle=False):
    ...
    if (STACK/'ops/releases/20260922').exists(): raise RuntimeError('Canonical release directory already exists; reconcile first')
...
def healthy(s,image,timeout=360):
    ...
    while time.monotonic()<deadline:
        ...
        if d['State'].get('Health',{}).get('Status')=='healthy' and d['State']['Running']:
            ...
            return
        time.sleep(3)
    raise RuntimeError(s+': health timeout')
...
def restore_compose(m,state):
    ...
    run(git('add','--','compose.yaml')); run(git('commit','-m','rollback: fleet release 20260922 image selectors'))
...
def install_source(m):
    destination=STACK/'ops/releases/20260922'
    if destination.exists(): raise RuntimeError('Release source already exists')
    stage=destination.with_name('20260922.staging')
...
def apply(m):
    ...
    try:
        ...
        run(git('add','--','compose.yaml','ops/releases/20260922')); run(git('commit','-m','release: Agent 0.21.4 and WebUI approval repair'))
        for s in ORDER:
            probe(s,'idle')
