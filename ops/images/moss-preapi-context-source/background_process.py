...
def active_stream_id_for_session(session_id: str) -> Optional[str]:
    ...
    stream_id. A session normally has one live run; cancel/reconnect can
    briefly hold multiple worker-lifecycle rows, so the lookup selects the
    first candidate that still has an attachable STREAMS transport.
    """
    from api import config as _cfg
        ...
        with _cfg.ACTIVE_RUNS_LOCK:
            for _stream_id, meta in (_cfg.ACTIVE_RUNS or {}).items():
                if isinstance(meta, dict) and meta.get("session_id") == session_id:
                    candidate_stream_ids.append(str(_stream_id))
        if candidate_stream_ids:
            # ACTIVE_RUNS follows the worker lifecycle and intentionally survives
            # briefly while a cancelled worker unwinds. Session-stream recovery,
            # however, may only advertise a stream that still has a live chat
            # transport. This matches /api/chat/stream/status and prevents a
            # terminal stream from being reattached in a cancel/reconnect loop.
            with _cfg.STREAMS_LOCK:
                for candidate_stream_id in candidate_stream_ids:
                    if candidate_stream_id in _cfg.STREAMS:
                        return candidate_stream_id
    except Exception:
        logger.debug(
