...
- Validated candidate image ID: `sha256:53a70a12a0f1fb1ea46489d7e5f772af5175edcadbd070404e87f4fde1adf3e3`
...
The build must fail closed if the base tag no longer resolves to the expected image ID. The overlay changes only the context indicator: no fixed 128K default, and percentage is shown only when prompt occupancy and an explicit context window are both known.
