...
def profile_proxy_public_entries(config_data=None, environ: dict[str, str] | None = None) -> list[dict]:
    ...
    return sorted(public, key=lambda p: p["name"])
...
def _run_gateway_chat_streaming(
    ...
    *,
    ...
    try:
        ...
        remote_profile = (selected_gateway or {}).get("remote_profile")
        ...
        put_gateway_event(
            "apperror",
            _gateway_http_error_event(exc, err_body, api_key_configured=bool(locals().get("api_key") or _gateway_api_key())),
        )
    except Exception as exc:
