...
class _RunLaunch:
    ...
    memory_admission: Optional[Dict[str, Any]] = None  # server-verified, never request-body attribution
...
async def _handle_runs(self, request: "web.Request", *, _api_server) -> "web.Response":
    ...
    launch = _RunLaunch(
        ...
        turn_author=turn_author, memory_admission=api_admission.get())
...
def _run_agent_sync(self, run: _RunLaunch, agent, approval_notify, *, _api_server):
    ...
    with self._profile_scope(run.request_profile), admission_scope(run.memory_admission):
        try:
