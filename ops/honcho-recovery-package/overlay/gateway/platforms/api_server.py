        ...
        async def profile_prefix_middleware(request: "web.Request", handler):
            ...
            try:
                with self._profile_scope(profile):
                    ...
                    from agent.moss_memory_gate import api_admission, request_admission
                    memory_token = api_admission.set(await request_admission(request, resolved_profile))
                    try:
                        return await handler(request)
                    finally:
                        api_admission.reset(memory_token)
