# Recovery: manual Honcho cutover for MEDIA
...
and `verify`. It must run as root from `/root/honcho-recovery-candidate` on MEDIA.
...
Luna binding, paired Telegram identity, WebUI password capability and preserved
`moss-rodolfo` workspace. Authorized channel rows are retained. The exact reviewed
empty diagnostic `private` row is accepted but never counts as ingestion.
Unknown origins or authors fail closed. Drift is a refusal, not a patch refresh.
...
The test requires two new conversations per channel after activation: write a
real preference in Telegram, recall it in a new WebUI chat and save another,
recall the second in a new Telegram conversation, and recall the first again in
another WebUI chat. Use Honcho tools, not Markdown or session history. Connection
alone is never acceptance. Channel API readback proves ingestion, while correct
answers without supplying the expected facts are attested by the operator.
...
If acceptance fails after ingestion, service recovery is automatic. A retry may
preserve recognized human-channel data after baseline image/config verification;
unknown origins still require reconciliation. Each attempt keeps a separate
encrypted snapshot and transaction journal. Never erase data to pass preflight.
The script also restores dreaming to disabled if failure occurs before committed
legacy cleanup. It does not claim automatic recovery after arbitrary host crash.
