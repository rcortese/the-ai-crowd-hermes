...
probe() { docker exec -i "$MOSS" "$PY" - "$@" < "$ROOT/runtime_probe.py"; }
check_installed() {
...
preflight() {
  ...
  probe baseline
  docker exec -i honcho-database-1 psql -U postgres -d postgres < "$ROOT/verify_recovery.sql"
...
docker exec -i "$CLONE" psql -U postgres -d postgres < "$ROOT/verify_recovery.sql" > "$STATE/fresh-rehearsal.txt"
docker exec "$CLONE" psql -U postgres -d postgres -v ON_ERROR_STOP=1 -c "UPDATE workspaces SET metadata='{}'::jsonb WHERE name='moss-rodolfo';" >> "$STATE/fresh-rehearsal.txt"
if docker exec -i "$CLONE" psql -U postgres -d postgres < "$ROOT/verify_recovery.sql" > "$STATE/fresh-negative.txt" 2>&1; then
  printf 'Teste negativo de proveniência do workspace falhou.\n' >&2; exit 1
fi
docker exec "$CLONE" psql -U postgres -d postgres -v ON_ERROR_STOP=1 -c "UPDATE workspaces SET metadata='{\"purpose\":\"Fresh direct Rodolfo-Moss conversational memory\",\"legacy_import\":false}'::jsonb WHERE name='moss-rodolfo';" >> "$STATE/fresh-rehearsal.txt"
docker exec -i "$CLONE" psql -U postgres -d postgres < "$ROOT/verify_recovery.sql" >> "$STATE/fresh-rehearsal.txt"
...
compose config --quiet
stackgit add -- compose.yaml ops/honcho-recovery-package
